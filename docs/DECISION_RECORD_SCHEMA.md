# Decision Record Schema - Aurora v1.4.0
...
- a `v1.4.0` não entrega schema novo nem motor novo; ela adiciona orientação de remote Flatpak explícito dentro de `source_clarification`, fora do `decision_record`, preserva a clarificação da `v1.3.0`, a orientação da `v1.2.0`, herda da `v1.0.0` a seam limitada em `facts.local_model` com provider real canônico e preserva `host_maintenance.atualizar` sem relaxar o contrato central.
