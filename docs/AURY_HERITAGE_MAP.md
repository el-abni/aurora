# Aury Heritage Map - Aurora v0.3.0

## Objetivo

Este mapa registra o que a Aurora herdou da Aury, o que foi refatorado e o que não migrou para o contrato público atual da `v0.3.0`.
...
| Classificação mínima de domínio | partes de `python/aury/analyzer.py` | reescrito no estilo Aurora | `host_package`, `user_software` e fonte explícita AUR sob contrato próprio |
...
- qualquer expansão pública além de `host_package`, AUR explícito e `user_software` via `flatpak`.

## Regra de proveniência

A Aury continua sendo a origem do patrimônio funcional herdado.

A Aurora não é rename da Aury e não é clean room. Ela é uma reencarnação disciplinada desse patrimônio sob contrato próprio, agora com `host_package` e `user_software` como domínios públicos distintos e AUR explícito como fonte separada dentro do escopo do host.
